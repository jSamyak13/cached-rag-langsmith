...
from crag_with_langsmith_tracing.backend.api.rag.schemas import RAGResponse
from langchain.agents import create_agent
from langgraph.graph import StateGraph, START, END
from langgraph.graph.message import add_messages
from langgraph.checkpoint.mongodb import MongoDBSaver
from crag_with_langsmith_tracing.backend.clients.mongodb_client import mongo_client
from crag_with_langsmith_tracing.backend.services.rag.chat_history import save_chat_log
from langchain.tools import tool
from langchain_core.messages import AIMessage
from crag_with_langsmith_tracing.backend.config.settings import settings
from crag_with_langsmith_tracing.backend.services.rag.cache import get_docs, save_docs, get_exact_cache, save_exact_cache
from crag_with_langsmith_tracing.backend.services.rag.embeddings import create_or_load_embeddings
from langchain_openai import ChatOpenAI
from crag_with_langsmith_tracing.backend.prompts.prompt import RAG_PROMPT
...
def retrieve_docs(query: str, config: RunnableConfig) -> str:
    """
    ...
    try:
        ...
        thread_id = config.get("configurable", {}).get("thread_id", "anonymous:default")
        user_id = thread_id.split(":")[0] if ":" in thread_id else "anonymous"
        
        cached_docs = get_docs(query, user_id)
        if cached_docs:
        ...
        save_docs(query, contents, user_id)
        return "\n\n".join(contents)
...
def run_agent(state: State, config: RunnableConfig):
    try:
        ...
        cached_answer = get_exact_cache(query, user_id)
        if cached_answer:
        ...
        save_exact_cache(query, answer_dict, user_id)
        return {
            "messages": [
...
def run_query(query: str, thread_id: str, user_id: str) -> dict:
    if not graph:
    ...
    try:
        ...
        namespaced_thread = f"{user_id}:{thread_id}"
        ...
        save_chat_log(thread_id, query, final_response.get("answer", ""), user_id)
